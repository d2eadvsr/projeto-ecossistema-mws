migrate(
  (app) => {
    ...
    let dentistProfile
    try {
      dentistProfile = app.findFirstRecordByData('dentists', 'user', drUser.id)
    } catch (_) {
      dentistProfile = new Record(dentists)
      dentistProfile.set('user', drUser.id)
    ...
    let patientProfile
    try {
      patientProfile = app.findFirstRecordByData('patients', 'user', ptUser.id)
    } catch (_) {
      patientProfile = new Record(patients)
      patientProfile.set('user', ptUser.id)
    ...
    // 5. Case
    try {
      app.findFirstRecordByData('clinical_cases', 'patient', patientProfile.id)
    } catch (_) {
      const clinicalCase = new Record(cases)
      clinicalCase.set('dentist', dentistProfile.id)
      clinicalCase.set('patient', patientProfile.id)
  ...
  (app) => {
    ...
    try {
      const record = app.findAuthRecordByEmail('_pb_users_auth_', 'patient@magicwire.com')
      app.delete(record)
    } catch (_) {}
    try {
      const record = app.findAuthRecordByEmail('_pb_users_auth_', 'lab@magicwire.com')
      app.delete(record)
    } catch (_) {}
  },
)
